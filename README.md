...
Проект реализует микросервисное приложение "Онлайн-кафе" (систему управления пользователями, биллингом, 
заказами, уведомлениями, складом и доставкой).
Данный документ описывает развёртывание микросервисного приложения Online Cafe в Kubernetes (Minikube)
с использованием Helm. Система состоит из шести сервисов: user, billing, notification, order, stock, delivery. 
...
1. Создаётся заказ со статусом PENDING (если есть товары на готовку) или CONFIRMED (если всё в наличии).
2. Последовательно вызываются шаги:
 - списание средств в billing,
 - резервирование товаров в stock,
 - резервирование слота доставки в delivery.
3. При успехе статус меняется на CONFIRMED, корзина очищается.
4. При сбое любого шага выполняются компенсирующие действия в обратном порядке (возврат средств, освобождение товаров, 
освобождение слота).
Это обеспечивает атомарность операции создания заказа без распределённых блокировок.

Этот подход обеспечивает атомарность операции создания заказа без распределённых блокировок, соответствует **SAGA**, 
**SOLID** (единая ответственность оркестратора) и **KISS** (явная последовательность шагов).
...
## Тестирование
### Postman‑коллекция
Функциональные тесты (Postman)
Для тестирования подготовлены следующие коллекции Postman (находятся в корне репозитория):

e2e-tests.json – полный E2E‑сценарий (регистрация, пополнение баланса, добавление в корзину, 
создание заказа с ожиданием, подтверждение, доставка, получение).

user-billing-order-notification.postman_collection.json – базовый сценарий user → billing → order → notification.
(создание пользователя, пополнение счёта, создание заказа с достаточными средствами, проверка баланса и уведомления,
создание заказа с недостаточными средствами, повторная проверка баланса и уведомления)

test-saga.json – тестирование Saga (успешный заказ, недостаток средств, отсутствие товара).

idempotency-key.json – проверка идемпотентности создания заказа.

postman_environment.json – переменные окружения (URL сервисов).

Запуск коллекций с помощью Newman (установите глобально: npm install -g newman):

```bash
# Базовый сценарий
newman run user-billing-order-notification.postman_collection.json -e postman_environment.json
```
```bash
# Сценарий Saga
newman run test-saga.json -e postman_environment.json
```
```bash
# Идемпотентность
newman run idempotency-key.json -e postman_environment.json
```
```bash
# Полный E2E‑тест
newman run e2e-tests.json -e postman_environment.json
...
![Результаты выполнения тестов saga](e2e-test.png)

## Структура проекта

OTUS_Online_Cafe/
├── user/                  # Сервис управления пользователями (порт 8000)
├── billing/               # Биллинг (порт 8001)
├── notification/          # Уведомления (порт 8002)
├── order/                 # Заказы – оркестратор Saga (порт 8003)
├── stock/                 # Управление складом (порт 8004)
├── delivery/              # Доставка (порт 8005)
├── e2e-tests.json
├── idempotency-key.json
├── postman_environment.json
├── test_saga.json
├── user-billing-order-notification.postman_collection.json
└── README.md
...
- build.gradle, settings.gradle (Spring Boot 3.2, Java 21),

- исходный код (src/main/java),

- конфигурацию (src/main/resources/application.yml),

- Helm‑чарт в charts/<service> для развёртывания в Kubernetes.
- 
## Заключение
После выполнения всех шагов вы получите полностью работающее микросервисное приложение «Онлайн-кафе», 
готовое к функциональному и нагрузочному тестированию. Все сервисы взаимодействуют через HTTP, 
используют паттерн Saga для обеспечения согласованности данных и идемпотентности для безопасного 
повторения запросов.